...
export const calculateAvailableCycles = (ns, nodes) => {
    let total = 0;
...
async function expFarm(ns, hackingNodes, stopTime) {
    while (new Date().getTime() < stopTime) {
        ...
        if (!DEBUG.dryrun) {
            for (const node of activeNodes) {
                executeAttackAction(ns, settings.attackScripts[2], node.host, settings.expFarmTarget, node.availableCycles, 0);
            }
        }
        if (DEBUG.expFarm) {
...
export function executeAttackAction(ns, action, attacker, target, threads, delay) {
    if (!DEBUG.dryrun) {
...
const locateTargets = (ns, servers, capacity) => {
...
export const hackThreadsNeededToSteal = (ns, target, percent) => {
        ...
        for (const target of targets) {
            ...
            for (const node of hackingNodes) {
                ...
                if (cyclesNeeded.weaken > 0 && node.availableCycles > 0) {
                    ...
                    executeAttackAction(ns, settings.attackScripts[2], node.host, target.host, count, 0);
                    cycles -= count;
                    ...
                    if (DEBUG.attackActions) {
                        ns.tprintf(debugAttackActionFormat, node.host, target.host, "weaken", count, cyclesNeeded.weaken, node.availableCycles, cycles);
                    }
                }
                if (cyclesNeeded.grow > 0 && node.availableCycles > 0) {
                    const count = Math.min(cyclesNeeded.grow, node.availableCycles);
                    executeAttackAction(ns, settings.attackScripts[1], node.host, target.host, count, actionTimes.growDelay);
                    cycles -= count;
                    node.availableCycles -= count;
                    cyclesNeeded.grow -= count;
                    longestWait = Math.max(longestWait, actionTimes.growDelay + actionTimes.grow);
                    if (DEBUG.attackActions) {
                        ns.tprintf(debugAttackActionFormat, node.host, target.host, "grow", count, cyclesNeeded.grow, node.availableCycles, cycles);
                    }
                }
                if (cyclesNeeded.additionalWeakens > 0 && node.availableCycles > 0) {
                    ...
                    executeAttackAction(ns, settings.attackScripts[2], node.host, target.host, count, actionTimes.additionalWeakenDelay);
                    cycles -= count;
                    node.availableCycles -= count;
                    ...
                    if (DEBUG.attackActions) {
                        ns.tprintf(debugAttackActionFormat, node.host, target.host, "+weaken", count, cyclesNeeded.additionalWeakens, node.availableCycles, cycles);
                    }
                }
            }
        }
...
async function runBatch(ns, target, hackingNodes, batch, delay, actionTimes, batchId) {
    ...
    for (const node of hackingNodes) {
        ...
        if (count > 0) {
            executeAttackAction(ns, settings.attackScripts[0], node.host, target, count, delay + actionTimes.hackDelay);
            hackCycles -= count;
            node.availableCycles -= count;
        }
    }
    ...
    for (const node of hackingNodes) {
        ...
        if (count > 0) {
            executeAttackAction(ns, settings.attackScripts[2], node.host, target, count, delay);
            weakenForHack -= count;
            node.availableCycles -= count;
        }
    }
    ...
    for (const node of hackingNodes) {
        ...
        if (count > 0) {
            executeAttackAction(ns, settings.attackScripts[1], node.host, target, count, delay + actionTimes.growDelay);
            growCycles -= count;
            node.availableCycles -= count;
        }
    }
    ...
    for (const node of hackingNodes) {
        ...
        if (count > 0) {
            executeAttackAction(ns, settings.attackScripts[2], node.host, target, count, delay + 20);
            weakenForGrow -= count;
            node.availableCycles -= count;
        }
    }
