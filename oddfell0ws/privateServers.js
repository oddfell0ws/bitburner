...
export async function main(ns) {
    ...
    switch (action) {
        ...
        case "analyze":
            ...
            steps.forEach(step => {
                ns.tprint(`${step.ram.toString().padStart(4)} GB\t - ${ns.nFormat(step.cost, "$0.000a").padStart(4)}\t - 25x ${ns.nFormat(step.cost*25, "$0.000a").padStart(4)}`);
            });
            break;
        case "plan": {
            const result = plan();
